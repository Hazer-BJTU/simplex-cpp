...
| --- | --- | --- | --- |
| `GET /agent/<session_id>/events?token=<t>` | events and inputs | persistent, reconnecting | `src/worker/connection.js` |
| `GET /agent/<session_id>/confirm?token=<t>` | one-shot confirmation | one request, one response, then close | `src/worker/confirmation.js` |
| anything else | — | rejected during the upgrade | `src/http/server.js` |
...
connection (`src/worker/confirmation.js`):
...
- `request_id` is generated by the hub when the panel does not supply one, and
  the payload is validated locally first (`src/protocol/messages.js`, mirroring
...
`src/launch/supervisor.js` owns worker processes. The order is always
**protocol first**:
...
| --- | --- | --- | --- |
| 1 | Accept worker text WebSockets, implement ping/pong and close | `src/http/server.js`, `src/worker/connection.js` | `ws` handles control frames; the hub adds an application ping interval. Routes are not reserved paths. |
| 2 | Associate connections with an authorized worker/session; treat payload senders as approval authorities | `src/http/auth.js`, `src/state/registry.js` | Per-session token in the query. Deployment owns the trust boundary around `/panel/ws` and `/api/*`; a non-loopback listener requires a panel token. |
| 3 | Generate valid request IDs, wait for outcomes, never retry side-effecting work | `src/state/registry.js`, `src/panel/api.js` | `sent` / `admitted` / `rejected` / `unknown` are all visible; there is no automatic resend. |
| 4 | Decode every event shape, treat text as untrusted, tolerate unknown fields | `src/protocol/events.js`, `hub/web/js/render.js` | Unknown events render generically; the panel never uses `innerHTML`. |
| 5 | Service confirmations concurrently, validate correlation, close promptly, retire disconnected prompts | `src/worker/confirmation.js` | Several prompts per session are supported; identity is judged as described above. |
| 6 | Target cancellation by run ID and wait for settlement | `src/panel/api.js` | The panel offers cancel only with a known run id, and shows `run_finished` as the settlement. |
| 7 | Keep reads flowing, track sequence gaps and overflow counters, send `status` after reconnect | `src/worker/connection.js` | Gaps, duplicates, `rejected_payloads`, and `storage_failed` are surfaced separately. |
| 8 | Surface run failure, storage failure, recovery phases, and connection loss distinctly | `hub/web/js/*`, `src/state/registry.js` | `tools`/`blocked` phases and `storage_failed` are called out; an unknown request outcome is never shown as success. |
