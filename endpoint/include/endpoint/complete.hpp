...
boost::asio::awaitable<model_io::MessageItem> complete_once(
    ...
    // ---- connect -----------------------------------------------------------
    // endpoint::connect() folds the create_connection_stream failure family
    // (the system_error group, plus a defensive stray) into
    // HttpRequestException{Stage::Connect} with the transport error code and
    // the endpoint's target/host context — the shared connect primitive
    // complete_once and fetch_once both ride on.
    connection_stream stream = co_await connect(executor, model_endpoint);
...
class complete : public retry_policy {
public:
    /**
     * @brief Construct the streaming retry engine for one executor.
     *
     * Forwards to the shared retry_policy — the backoff state and the
     * recoverability verdict complete and fetch both inherit — so the
     * parameter contract is retry_policy's. Delta (the reader's decoded
     * event type) rides on THIS class for the deduction reason in the class
     * doc; copy/move stay deleted through the base (a bound steady_timer
     * plus rolling retry state).
     */
    complete(
        boost::asio::any_io_executor executor,
        ...
        : retry_policy(std::move(executor), initial_backoff, max_backoff, max_retry_attempts)
    {}
