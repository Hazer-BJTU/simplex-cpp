 ...
 * none and the local legacy minimal alias disable), the public effort values
 * (none|low|high|max) and permissive compatibility/unknown-value pass-through,
 * the undocumented/deprecated
...
BOOST_AUTO_TEST_CASE(effort_vocabulary_passes_through_verbatim) {
    // Public enabled efforts and compatibility values all pass through.
    // This offline test asserts adapter policy, not server acceptance.
    ...
    // Unknown values also reach the server unchanged for validation.
